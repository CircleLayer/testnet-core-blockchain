...
task4(){
  ...
  if [[ $totalValidator -gt 0 ]]; then
    ...
    LINE='cd /root/testnet-core-blockchain/'
    if grep -Fxq "$LINE" /etc/profile
    then
      # code if found
      ...
      # code if not found
      echo -e '\ncd /root/testnet-core-blockchain/' >>/etc/profile
    fi

    LINE='bash /root/testnet-core-blockchain/node-start.sh --validator'
    if grep -Fxq "$LINE" /etc/profile
    then
      # code if found
      ...
      # code if not found
      echo -e '\nbash /root/testnet-core-blockchain/node-start.sh --validator' >>/etc/profile
    fi
  ...
  if [[ $totalRpc -gt 0 ]]; then
    ...
    LINE='bash /root/testnet-core-blockchain/node-start.sh --rpc'
    if grep -Fxq "$LINE" /etc/profile
    then
      # code if found
      echo -e "${ORANGE}autostart is already added"
      ...
      # code if not found
      echo -e '\nbash /root/testnet-core-blockchain/node-start.sh --rpc' >>/etc/profile
    fi
...
finalize(){
  ...
  # resource paths
  nodePath=/root/testnet-core-blockchain
  ...
  chown -R root:root /root/testnet-core-blockchain/chaindata
  chmod -R 755 /root/testnet-core-blockchain/chaindata
